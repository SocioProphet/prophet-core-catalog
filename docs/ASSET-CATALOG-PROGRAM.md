...
- Cross-repo write needs a **GitHub App installation token minted in CI** (never a PAT, never a per-consumer operator secret — all tokens minted in CI). The reusable workflow mints it per run from the estate's **shared ops App** (the same `GH_OPS_APP_ID` / `GH_OPS_APP_PRIVATE_KEY` org secrets `board-parity` / `estate-ci-health` use), scoping the token with `repositories: prophet-core-catalog` so it can write nowhere else. No catalog-specific secret exists (`CATALOG_APP_ID` / `CATALOG_APP_PRIVATE_KEY` are retired); consumers set nothing and forward the shared org secrets with `secrets: inherit`.
