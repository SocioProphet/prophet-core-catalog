...
| `regex-corpus.jsonl` | **2,275** distinct patterns; one record per pattern; identical patterns deduped, with every usage site listed in `sources[]` (these are the blast-radius edges). |
...
## Governance
- **Competitor-clean:** patterns referencing competitor models/vendors or AI key prefixes are quarantined out of this public corpus (`QUARANTINE-NOTE.md`). The validator fails a public dataset whose corpus is not `competitor_clean`.
